    ...
    @if( isset($field['options']) && is_array($field['options']) )
        ...
        @foreach ($field['options'] as $value => $label )
            ...
            <label class="radio-inline" for="{{$field['name']}}_{{$optionPointer}}">
                <input disabled data-field-toggle="{{ json_encode($field['hide_when']) }}" type="radio" id="{{$field['name']}}_{{$optionPointer}}" name="{{$field['name']}}" value="{{$value}}" {{$optionValue!=='' && $optionValue == $value ? ' checked': ''}}> {!! $label !!}
            </label>
