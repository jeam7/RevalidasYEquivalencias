    ...
    <div class="col-md-8 col-md-offset-2">
      <div class="">
        ...
        <table id="crudTable" class="box table table-striped table-hover display responsive nowrap m-t-0 dataTable dtr-inline" cellspacing="0">
            ...
            <tbody>
                <tr>
                  @if (count($entry->career) > 0)
                  @foreach ($entry->career as $careers)
                     <tr>
                          <td><a href="{{ url('admin/career/'.$careers->id) }}">{{$careers->name}}</a></td>
                          <td>
                            <a href="{{ url('admin/career/'.$careers->id.'/edit?school_from='.$entry->getKey()) }}" class="btn btn-xs btn-default"><i class="fa fa-edit" title="{{ trans('backpack::crud.edit') }}"></i> editar</a>
                            @if (!in_array(backpack_user()->type_user, [2,3,4]))
                              <a href="javascript:void(0)" onclick="deleteEntry(this)" data-route="{{ url('admin/career/'.$careers->id) }}" class="btn btn-xs btn-default" data-button-type="delete"><i class="fa fa-trash"></i> Eliminar</a>
                            @endif
                          </td>
                     </tr>
                  @endforeach
