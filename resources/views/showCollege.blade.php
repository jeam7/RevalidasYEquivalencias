    ...
    <div class="col-md-8 col-md-offset-2">
      <div class="">
        ...
        <table id="crudTable" class="box table table-striped table-hover display responsive nowrap m-t-0 dataTable dtr-inline" cellspacing="0">
            ...
            <tbody>
                <tr>
                  @if (count($entry->faculty) > 0)
                  @foreach ($entry->faculty as $faculties)
                     <tr>
                          <td><a href="{{ url('admin/faculty/'.$faculties->id) }}">{{$faculties->name}}</a></td>
                          <td>
                            <a href="{{ url('admin/faculty/'.$faculties->id.'/edit?college_from='.$entry->getKey()) }}" class="btn btn-xs btn-default"><i class="fa fa-edit" title="{{ trans('backpack::crud.edit') }}"></i> editar</a>
                            @if (!in_array(backpack_user()->type_user, [2,3,4]))
                              <a href="javascript:void(0)" onclick="deleteEntry(this)" data-route="{{ url('admin/faculty/'.$faculties->id) }}" class="btn btn-xs btn-default" data-button-type="delete"><i class="fa fa-trash"></i> Eliminar</a>
                            @endif
                          </td>
                     </tr>
                  @endforeach
                  @else
                      <td valign="top" colspan="2" class="dataTables_empty" style="text-align:center">No hay Facultades disponibles para esta Universidad</td>
                  @endif
